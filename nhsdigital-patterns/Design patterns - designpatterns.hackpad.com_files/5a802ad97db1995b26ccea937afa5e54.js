...
}};domline.addSurrogate=function(B,E,G,F,D){var A=domline.hashCode(B);content='<span class="inline-'+E+' gallery-wrapper" '+E+'="'+_htmlEscaped(B)+'"'+(B?' data-ace-gallery-urls="'+_htmlEscaped(B.replace(/ /g,"%20"))+'"':"")+'><div contenteditable="false" class="embed-'+A+'">';
var C=domline.embedhtml[A];if(C){content+=C;}else{content+='<img class="inline-img" src="/static/img/pixel.gif" contenteditable="false">';
}content+="</div></span>";if(C==undefined){domline.embedhtml[A]=false;F(B,function(J){J=J||"";domline.embedhtml[A]=J;
var L=G.querySelectorAll(".embed-"+A);for(var K=0;K<L.length;K++){var M=L[K];M.innerHTML=J;if(document&&D){var I=M.querySelectorAll("img, iframe");
for(var H=0;H<I.length;++H){if(I[H].addEventListener){I[H].addEventListener("load",D,false);}else{I[H].attachEvent("onload",D);
